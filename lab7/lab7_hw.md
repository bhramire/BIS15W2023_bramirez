...
```r
#NAs are stored as -999.0 values.
```
...
```r
#That's rather odd. They might be -999.0.
```
...
```r
#It makes sense for mammal data to be missing as mammals don't lay eggs! However, the bird and reptile data may just simply be missing if it wasn't gathered or if it wasn't relevant. Reptile eggs may be harder to find or weigh.
```
...
```r
#I believe they represent a false on whether a certain species lives in that classification of habitat(Fossorial, terrestrial, etc..). The only value that they take other than NA is 1 which I believe represents true. It should have been a factored variable if they really wanted it to be efficient. We could do that. 
```
