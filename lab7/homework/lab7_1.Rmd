...
##Not tidy because species are observations.
```
...
penguins %>% 
  ...
  summarise(
    avg_bill_length_mm = mean(bill_length_mm, na.rm =T),
    avg_bill_depth_mm = mean(bill_depth_mm, na.rm =T),
      avg_flipper_length_mm = mean(flipper_length_mm, na.rm =T),
      avg_body_mass_g = mean(body_mass_g, na.rm =T),
      n_sample = n()
  )
```
