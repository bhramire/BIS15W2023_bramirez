...
ui <- fluidPage(titlePanel("Log 10 Homerange by Taxon"),
  selectInput("x", "Select Taxon", 
              choice = c("birds","lake fishes", "lizards", "mammals", "marine fishes", "river fishes", "snakes", "tortoises", "turtles"),                  selected = "birds"), 
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    homerange %>%  
      ...
      ggplot(aes(x= log10.hra))+geom_density(fill= "navyblue",alpha =0.5)
  })
}
