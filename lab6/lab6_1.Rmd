...
```{r}
msleep %>% 
  summarize(mean_sleep_overall = mean(sleep_total),
            min_sleep_overall = min(sleep_total),
            max_sleep_overall = max(sleep_total),
            total = n())
```
...
```{r}
#NA means a data value is missing and its trying to do calculations with that value.
```
...
#They don't have recorded brain weight. Whales are massive.
```
