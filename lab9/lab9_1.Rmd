...
```{r}
ggplot(data = homerange, mapping = aes(y = log10.hra, x = log10.mass) )+ geom_point()
```
...
```{r}
ggplot(data = homerange, mapping = aes(y = log10.hra, x = log10.mass) )+ geom_jitter()
```

To add a regression (best of fit) line, we just add another layer.
```{r}
ggplot(data = homerange, mapping = aes(y = log10.hra, x = log10.mass) )+ geom_point() + geom_smooth(method=lm, se=T)
```

### Practice
1. What is the relationship between log10.hra and log10.preymass? What do you notice about how ggplot treats NA's?
```{r}
ggplot(data = homerange, mapping = aes(x = log10.hra, y = log10.preymass) )+ geom_point(na.rm=T) + geom_smooth(method = lm, se=T)
#It doesn't plot NA values so the graph looks empty.
```
...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+#Since we added in a y axis labeled, we used geom_col
  geom_col()
```
...
```{r}
mammals_hra %>% 
  select(common.name, genus, species, trophic.guild) %>% 
