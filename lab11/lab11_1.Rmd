...
```{r}
homerange %>% 
  ggplot(aes(x=class, y=log10.mass))+geom_boxplot()+labs(title="Ranges of log10 Mass per Class", x= "Class", y="Range of Mass")
```

2. Now, add a layer to this plot that fills color for each box by taxonomic class
```{r}
homerange %>% 
  ggplot(aes(x=class, y=log10.mass,group=taxon, fill=taxon))+geom_boxplot()+labs(title="Ranges of log10 Mass per Class", x= "Class", y="Range of Mass")
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.5, color = "black", fill = "darkolivegreen4", bins=40)+
...
homerange %>% 
  ggplot(aes(x = log10.mass))+
geom_histogram(alpha = 0.8, color = "black", fill = "grey", bins=20)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass))+
geom_density(alpha = 0.5, color = "black", fill = "navyblue")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass))+
geom_histogram(aes(y=after_stat(density)),alpha = 0.8, color = "black", fill = "grey", bins=20)+
  labs(title = "Distribution of Body Mass")+geom_density(alpha = 0.5, color = "black", fill = "navyblue")
```
...
homerange<-homerange %>% 
  mutate(range_category = case_when( log10.hra < 3.65 ~ 'very small',
                                     ...
                                     log10.hra >= 4.59 & log10.hra <= 6.016 ~ 'medium',
...
```{r}
homerange %>% 
  ggplot(aes(x=range_category, fill = class, ))+geom_bar(position = "dodge",alpha =0.75, color = "Black")+labs(title = "Distribution of log10.hra by Class", x= "Range Size", y= "Count of Individuals") + theme(plot.title = element_text(size = rel(1.5), hjust = 0.5) )
```
...
homerange %>% 
  ...
  ggplot(aes(x=class, y=log10.mass, fill=class))+ geom_boxplot()+ labs(title = "Distribution of log10.mass for Classes for small HRA", x= "Class", y= "Range of Log10 Mass")
```
