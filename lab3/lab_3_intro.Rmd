...
```{r}
PlantExperiment <- c(PlantHeight, PlantWeight)
PlantExperiment
```
```{r}
plant_data_matrix <- matrix(PlantExperiment, nrow = 5, byrow = F)
plant_data_matrix
```
```{r}
PlantLabels <- c("Plant 1", "Plant 2", "Plant 3", "Plant 4", "Plant 5")
PlantDataType <- c("Height", "Weight")
colnames(plant_data_matrix) <- PlantDataType
rownames(plant_data_matrix) <- PlantLabels
plant_data_matrix
```
