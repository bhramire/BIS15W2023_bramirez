...
names(spider_list)
```
##Practice: Access the Contra Costa County Data
```{r}
...
`inner_join`  
Where did customers 3 and 4 go? #Joins Data with matching values for 'by' variable
```{r}
...
`left_join`
Where did customer 4 go? Notice the missing data for customer 3. #Joins data and prioritizes the left/first dataset.
```{r}
...
`right_join`
Where did customer 3 go? Notice the missing data for customer 4. #Joins data and prioritizes right/second data set.
```{r}
right_exampleDF <- right_join(table_A, table_B, by="customer_ID")
right_exampleDF
```

`full_join` #Joins everything
```{r}
...
```{r}
dateformat1 <- ymd(dateformat1)

dateformat3 <- dmy(dateformat3)
dateformat4 <- mdy_hms(dateformat4)
dateformat5 <- ymd_hms(dateformat5)
```
```{r}
 mdy(dateformat2)
