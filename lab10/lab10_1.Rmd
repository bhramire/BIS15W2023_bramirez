...
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y=wean_mass))+geom_point()+scale_y_log10()
```
...
```{r}
elephants %>% 
  ggplot(aes(x=age, y=height)) + geom_point() + geom_smooth(method=lm, se=F)+labs(title="Elephant Age vs. Height",x="Age", y="Height")+ theme(plot.title=element_text(size=rel(1.25), hjust=0.5))
```
